...
//调用notice订阅消息
func (p *postman) subscribe(id int64, host string) error {
	req := &meta.SubscribeRequest{ID: id, Host: host}
	if p.notice == nil {
		return errors.New("error notice is nil")
	}

	resp, err := p.notice.Subscribe(context.Background(), req)
	if err != nil {
		return errors.Trace(err)
	}
...
//调用notice取消订阅消息
func (p *postman) unSubscribe(id int64, host string) error {
	req := &meta.UnSubscribeRequest{ID: id, Host: host}
	if p.notice == nil {
		return errors.New("error notice is nil")
	}

	resp, err := p.notice.UnSubscribe(context.Background(), req)
	if err != nil {
		return errors.Trace(err)
	}
